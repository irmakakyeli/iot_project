        ...
        if content_type == 'text':
            if msg['text'] == '/start':
                if chat_ID not in self.chat_ids:
                    ...
                    self.bot.sendMessage(chat_ID, "✅ You have successfully registered! Please use /subscribe to continue.")
            elif msg['text'] == '/subscribe':
                keyboard = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text='Start UV Monitoring', callback_data='full')]
                ])
                self.bot.sendMessage(chat_ID, 'Lets check your location now!', reply_markup=keyboard)
            ...
            try:
                ...
                response = requests.post("http://127.0.0.1:8081/register_user", json=reg_payload)
                if response.status_code == 200:
                    print(f"[REGISTER] {user_id} registered to catalog.")
                else:
                    print(f"[REGISTER] Failed with status {response.status_code}: {response.text}")
            except Exception as e:
                print(f"[REGISTER] Exception occurred: {e}")
